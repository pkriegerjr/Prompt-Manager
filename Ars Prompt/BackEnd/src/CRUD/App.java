...
public class App {
    ...
    // ── Resend API ───────────────────────────────────────────
    static final String RESEND_KEY  = "re_ADPYoftE_JJvhwhpR16MXdqB8dEBKrnWn";
    static final String EMAIL_FROM  = "onboarding@resend.dev";
    // Página de verificação — acessada via Tomcat na porta 8080
    ...
    static void enviarLinkVerificacao(String destino, String nome, String token, boolean isAdmin) {
        try {
            ...
            // HTML sem caracteres especiais para evitar quebra no JSON
            String html =
            ...
            // Escapa o HTML para JSON manualmente de forma segura
            ...
            String jsonBody = "{" +
                "\"from\":\"" + EMAIL_FROM + "\"," +
                "\"to\":[\"" + destino + "\"]," +
                "\"subject\":\"ars-prompt - Verifique sua conta\"," +
                "\"html\":\"" + htmlEscapado + "\"" +
                "}";
            ...
            if (res.statusCode() == 200 || res.statusCode() == 201) {
                ...
                System.err.println("[EMAIL] Falha (" + res.statusCode() + "): " + res.body());
            }
        } catch (Exception e) {
    ...
    static void handleTornarAdmin(HttpExchange ex) throws Exception {
        if (!"POST".equals(ex.getRequestMethod())) { respText(ex,405,"Metodo nao permitido"); return; }
        String body = body(ex);
        int id = Integer.parseInt(jstr(body,"id"));
